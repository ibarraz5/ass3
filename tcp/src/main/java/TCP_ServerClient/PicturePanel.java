...
public class PicturePanel extends JPanel {
	...
	private enum States {
		// no game started
		NotStarted, // game started, but has no image and therefore doesn't know size
		InGameNoImage, // game fully initialized
		InGameWithImage
	}
	...
	public boolean insertImage(String fname, int row, int col) throws IOException, InvalidCoordinateException {
		...
		if (row < 0 || col < 0 || row >= 0 && labels.length <= row || labels[row].length <= col) {
