...
public class NetworkUtility {
	...
	public static void Send(DatagramSocket sock, InetAddress addr, int port, byte... bytes) throws IOException {
		...
		while (offset < bytes.length) {
			...
			byte[] totalBytes = NetworkUtility.intToBytes(packetsTotal);
			byte[] currentBytes = NetworkUtility.intToBytes(packetNum);
			byte[] lengthBytes = NetworkUtility.intToBytes(length);
	...
	static class Packet {
		...
		public Packet(DatagramPacket packet) {
			...
			Total = NetworkUtility.bytesToInt(totalBytes);

			byte[] currentBytes = new byte[4];
			System.arraycopy(packet.getData(), 4, currentBytes, 0, 4);
			Current = NetworkUtility.bytesToInt(currentBytes);

			byte[] lengthBytes = new byte[4];
			System.arraycopy(packet.getData(), 8, lengthBytes, 0, 4);
			Length = NetworkUtility.bytesToInt(lengthBytes);
